...
class WebpackFtpUploadPlugin {
  ...
  apply(compiler) {
    ...
    // compiler.plugin('done', compilation => { // old Webpack
    compiler.hooks.done.tap('webpack-ftp-upload', stats => {
  ...
  upload(local, obj) {
    Client.scp(local, obj, err => {
      ...
      console.log(
        `Uploaded ${local} successfully in ${this.endTime - this.startTime}ms`
          .green
      );
    });
  }
